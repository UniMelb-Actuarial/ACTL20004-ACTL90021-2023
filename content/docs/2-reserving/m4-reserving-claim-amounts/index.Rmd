...
[^1]: References: Chapter 3 & 4 of @Tay00 | $\; \rightarrow$  [\textcolor{blue}{\underline{latest slides}}](https://gim-am3.netlify.app/output/23-Top-M4-lec.pdf) 
...
This is discussed/applied in Section 4.4 of @Tay00.
...
$$\begin{aligned}
\alpha(i) &= \frac{\lambda(0)}{\lambda_0} \alpha^*(i) (1+f)^i, \\
\mu(j) &= \mu^*(j) (1+f)^j, \text{ with}\\
\sum_j \mu^*(j) &= 1.
\end{aligned}$$
...
- Note that the factors are no longer all $>1$, which leads to a new smoothing method, namely here a 3-period moving average; see (3.61) in @Tay00.
