...
- a `\(period\)` be some measure of time length, such as a month, quarter or year
...
(again, superimposed inflation is out of scope)
...
- **unallocated** loss adjustment expenses (ULAE)
  - these cannot be easily linked to specific claims (think overhead)
  - these are typically ont in the `\(C(i,j)\)` and will require specific consideration if they are to be included.
  - denote
    $$ e(i,j)$$
...
- In general this is unknown, and is now the question is how to estimate this, and how much to set aside for covering those liabilities
- I makes sense that you would reserve its expectation plus some margin, so that we define:
  - **Expected outstanding loss liability**: this is the expectation, also called **central estimate**
  - **Prudential margin** or **risk margin** is the margin you add to the central estimate to allow for estimation error
- In the end we have:
...
Nowadays, one also typically have:
...
These can be used for improving reserving, but this is outside scope of this subject.
